...
class Shop extends Component {
  constructor(props) {
    ...
    if (typeof window !== "undefined") {
      this.state = {
        ...
        internet_package_name:
          localStorage.getItem("internet_package_name") || "",
        internet_package_fee:
          parseInt(localStorage.getItem("internet_package_fee")) || 0,
        internet_package_speed:
          parseInt(localStorage.getItem("internet_package_speed")) || 0,
        service_contract: localStorage.getItem("service_contract") || "2YR",

        internet_hardware_id:
          parseInt(localStorage.getItem("internet_hardware_id")) || 0,
        internet_hardware_description:
          localStorage.getItem("internet_hardware_description") || "",
        internet_hardware_monthly_fee:
          parseFloat(localStorage.getItem("internet_hardware_monthly_fee")) ||
          0,
        internet_hardware_one_time_fee:
          parseFloat(localStorage.getItem("internet_hardware_one_time_fee")) ||
          0,
        ...
        tv_hardware_one_time_fee:
          parseFloat(localStorage.getItem("tv_hardware_one_time_fee")) || 0,

        phone_package_id:
          parseInt(localStorage.getItem("phone_package_id")) || 0,
        phone_package_name: localStorage.getItem("phone_package_name") || "",
        phone_package_fee:
          parseInt(localStorage.getItem("phone_package_fee")) || 0,
        phone_hardware_id:
          parseInt(localStorage.getItem("phone_hardware_id")) || 0,
        phone_hardware_name: localStorage.getItem("phone_hardware_name") || "",
        phone_hardware_one_time_fee:
          parseFloat(localStorage.getItem("phone_hardware_one_time_fee")) || 0,
        ...
        phone_port_authorization:
          localStorage.getItem("phone_port_authorization") || "",
        selected_installation_date:
          localStorage.getItem("selected_installation_date") || "ASAP",
        selected_installation_time:
          localStorage.getItem("selected_installation_time") || "Any Time",
        promo_code_name: "",
        promo_code_message: "",
        promo_code_type: "",
        promo_code_value: 0,
    ...
    this.updatePhonePortAuthorization = this.updatePhonePortAuthorization.bind(
      this
    )
      ...
      case "/fr/phone/":
        ...
        if (this.state.phone_port === "YES") {
          ...
          if (this.state.validation_pass_authorized_to_port_number !== true) {
            console.log(
              "this.state.validation_pass_authorized_to_port_number:",
              this.state.validation_pass_authorized_to_port_number
            )
            validation_error = true
  ...
  setDefaultInternetHardware() {
    ...
    if (this.props.packages.hardware) {
      let hardware = this.props.packages.hardware.find(
        option =>
          option.description === "Rent to own Modem and FREE WiFi Router"
      )
      ...
      localStorage.setItem(
        "internet_hardware_monthly_fee",
        hardware.monthly_fee
      )
      localStorage.setItem(
        "internet_hardware_one_time_fee",
        hardware.initial_cost
      )
    }
  }
  ...
  selectPhoneHardware(e) {
    let hardware = this.props.packages.hardware.find(
      option => option.id === parseInt(e.currentTarget.value)
    )
  ...
  addBaseInternetPackage(option) {
    ...
    if (option === "PHONE") {
      internet_package =
        this.props.packages.internet.find(item => item.download_speed >= 5) ||
        this.props.packages.internet[0]
    } else {
      internet_package =
        this.props.packages.internet.find(item => item.download_speed >= 10) ||
        this.props.packages.internet[1]
    }
    ...
    localStorage.setItem(
      "internet_package_speed",
      internet_package.download_speed
    )
  ...
  setInternetPackage(e) {
    let internet_package = this.props.packages.internet.find(
  ...
  setPhonePackage(e) {
    let phone_package = this.props.packages.phone.find(
      item => item.id === parseInt(e.currentTarget.value)
    )
    let hardware = this.props.packages.hardware.find(
      option => option.name === "Grandstream GS-HT801"
    )
  ...
  setServiceContract(e) {
    ...
    let internet_package = this.props.packages.internet.find(
      item => item.id === parseInt(this.state.internet_package_id)
    )
    let fee = 0
  ...
  setTVPackage(e) {
    let tv_package = this.props.packages.tv.find(
      item => item.id === parseInt(e.currentTarget.value)
    )
  ...
  render() {
    ...
    const order_data = {
      ...
      verification_passed_service_status: this.state
        .verification_passed_service_status,
      validation_pass_port_number_option: this.state
        .validation_pass_port_number_option,
      validation_pass_authorized_to_port_number: this.state
        .validation_pass_authorized_to_port_number,
      validation_pass_internet_hardware_option: this.state
        .validation_pass_internet_hardware_option,
    }
        ...
        <div className="lg:flex">
          <div className="lg:flex-1 px-3">
            <Router>
              ...
              <OrderReview
                path={
                  this.props.lang === "en" ? "order_review" : "fr/order_review"
                }
                lang={this.props.lang}
                order_data={order_data}
