  ...
  return (
    <>
      ...
      <Layout title={data.site.siteMetadata.title} lang="en">
        <Phone
          intro={data.allPrismicHomeP.edges[0].node.data.intro_content[0]}
          section={data.allPrismicHomeP.edges[0].node.data.section}
          lang="en"
        />
      </Layout>
    </>
...
export const query = graphql`
  query PhonePageQuery {
    ...
    allPrismicHomeP(filter: { lang: { eq: "en-us" }, tags: { in: "phone" } }) {
      edges {
        node {
          data {
            section {
